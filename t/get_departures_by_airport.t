...
subtest 'Flight data is available' => sub {
    ...
    my $flights_raw = $raw->get_departures_by_airport( $airport, $then, $now );
    set_response( one_flight() );
    my $flights_objects = $objects->get_departures_by_airport( $airport, $then, $now );
...
subtest 'Bad time intervals' => sub {
    my $then = $now + 3600;
    set_response( one_flight() );
    throws_ok { $raw->get_departures_by_airport( $airport, $then, $now ) }
